...
class ComplexityClassifier(MLClassifierBase):
    ...
    PROMPT = (
        "你是一个查询复杂度分析专家。请分析用户输入的复杂度，仅返回 \"simple\" 或 \"complex\"。\n\n"
        "## 第一步：判断是否为独立问题\n\n"
        "首先判断用户输入是否是一个可以独立理解的问题或陈述。\n\n"
        "**独立问题**（可以独立理解并回答）：\n"
        "- 正常的提问，如\"今天天气怎么样\"、\"帮我分析投资策略\"\n"
        "- 问候语或礼貌用语，如\"你好\"、\"谢谢\"、\"再见\"\n"
        "- 明确的指令或请求，如\"停止\"、\"继续\"\n"
        "- 可以独立完成的简单任务\n\n"
        "**非独立问题**（是对上一轮对话的信息补充）：\n"
        "- 用户仅补充信息，如提供客户号\"001\"、身份证号、账号等\n"
        "- 用户回答模型的追问，如\"北京\"、\"是的\"、\"三天前\"\n"
        "- 简短的数据输入，如\"123456\"、\"张三\"\n"
        "- 明显是对前一轮问题的延续而非新问题\n\n"
        "如果是**非独立问题**，直接返回 **complex**（因为这是上一轮复杂问题的延续）。\n\n"
        "## 第二步：判断复杂度（仅对独立问题）\n\n"
